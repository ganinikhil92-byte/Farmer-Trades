      ...
      {loading ? (
        ...
        <div className="table-container">
          <table>
            ...
            <tbody>
              {listings.length === 0 ? (
                <tr><td colSpan={8} style={{ textAlign: 'center', padding: '2rem', color: 'var(--color-text-muted)' }}>No fruit listings yet.</td></tr>
              ) : (
                listings.map((l, i) => (
                  <tr key={l.id}>
                    <td>{i + 1}</td>
                    <td style={{ width: '70px', padding: '0.4rem 0.6rem' }}>
                      <img
                        ...
                        onError={(e) => {
                          (e.currentTarget as HTMLImageElement).src = FALLBACK_PHOTOS.fruit;
                        }}
                      />
                    </td>
